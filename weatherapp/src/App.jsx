...
import { convertTemperature, getWindDirection, getHumidityValue, getVisibilityValue} from './components/Helper'
import { HumidityIcon, VisibilityIcon, SunriseIcon, SunsetIcon } from './components/Icons'
...
const fetchSuggestions = async (query) => {
  ...
  const fetchWeatherData = async (url,name = '') => {
    ...
    try{
      ...
      if(!response.ok) throw new Error((await response.json()).message || 'Failed to fetch weather data');
  ...
  return (
    ...
    <WeatherBackground weather={getWeathercondition()}>
        ...
        text-white w-full border border-white/30 relative z-10">
          ...
          {!weather ? (
            <form onSubmit={handleSearch} className="flex flex-col relative">
              <input value={city} onChange={(e) => setCity(e.target.value)} placeholder='Enter City or Country' className="mb-4 p-3 rounded border border-white bg-transparent text-white placeholder-white focus:outline-none 
              focus:border-blue-300 transition duration-300"></input>
              {suggestion.length>0 && (
